...
const MessagesScreen = () => {
  ...
  const getUserDataFromAsync = async () => {
    const userInfoData = await getDataFromAsync('@userInfo')
    imageStore.setSenderId(userInfoData?.empId)
  ...
  const onSend = useCallback((messageArray = []) => {
    ...
    const myMessage = {
      ...
      receverId: imageStore?.receverId,
    ...
    firestore()
      ...
      .doc('Pixako')
      .collection('Chats')
      .doc('muhammadumair.pixako@gmail.com')
      .collection('messages')
      .add({
        ...myMessage,
  ...
  return (
    <View style={{ flex: 1 }}>
      ...
      <GiftedChat
        ...
        textInputStyle={{ color: colors.blackColor, fontSize: getSize(14), lineHeight: 20 }}
        user={{
          _id: userData ? userData.empId : '',
          avatar: imageStore ? imageStore.image : '',
          name: userData ? userData?.empName : ''
        }}
        renderSend={props => {
          return (
